# Cosmos-based Node Ansible Setup Plus Several Supporting Playbooks For Celestia
...
```bash
ansible-playbook -i inventory_testnet.ini main.yml  -e "protocol=celestia" -e "target=celestia_test"
```

It is not recommended for you to sync from Block 1. Rather, please [state-sync mocha-4](https://services.kjnodes.com/testnet/celestia/) or start from a [snapshot](https://polkachu.com/tendermint_snapshots).

#### Host Variables

Take a look at the `inventory.sample` file. You will see an example `celestia` group with 3 different hosts: `celestia_main`, `celestia_backup`, and `celestia_relayer`. Each host will have the following variables:
