# Cosmos-based Node Ansible Setup Plus Several Supporting Playbooks For AtomOne

Based on [polkachu cosmos-validators](https://github.com/polkachu/cosmos-validators/)

## Design Philosophy

1. Support both mainnet
...
```bash
ansible-playbook -i inventory_testnet.ini main.yml  -e "protocol=atom_one" -e "target=atom_one_test"
```

It is not recommended for you to sync from Block 1. Rather, please [state-sync govgen-1](https://explorer.equinoxdao.xyz/govgen/statesync) .

#### Host Variables

Take a look at the `inventory.sample` file. Each host will have the following variables:
...
```bash
ansible-playbook -i inventory.ini main.yml -e "protocol=atom_one" -e "target=HOST_NAME"  -e "state_sync=true/false" -e "launch=true"
```
