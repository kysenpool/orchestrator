# Cosmos-based Node Ansible Setup Plus Several Supporting Playbooks For Agoric
...
```bash
ansible-playbook -i inventory_testnet.ini main.yml  -e "protocol=agoric" -e "target=agoric_test"
```

It is not recommended for you to sync from Block 1. Rather, please [state-sync enerynet-testnet](https://emerynet.explorer.agoric.net/agoric/statesync) or start from a [snapshot](https://polkachu.com/tendermint_snapshots).

#### Host Variables

Take a look at the `inventory.sample` file. You will see an example `agoric` group with 3 different hosts: `agoric_main`, `agoric_backup`, and `agoric_relayer`. Each host will have the following variables:
